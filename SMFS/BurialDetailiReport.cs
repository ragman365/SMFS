...
{
    /****************************************************************************************/
    public partial class BurialDetailReport : DevExpress.XtraEditors.XtraForm
    {
        ...
        /****************************************************************************************/
        private void BurialDetailReport_Load(object sender, EventArgs e)
        {
            chkInclude.Hide();
        ...
        private void printableComponentLink1_CreateMarginalHeaderArea(object sender, CreateAreaEventArgs e)
        {
            ...
            if (dgv2.Visible)
                title = "Burial Detail Locations";
            else if (dgv3.Visible)
                title = "Burial Detail Arrangers";
        ...
        private void tabControl1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (current.Name.ToUpper() == "TABPAGE2")
                BuildBurialDetail();
            else if (current.Name.ToUpper() == "TABPAGE3")
                BuildBurialArrangers ();
        }
        /****************************************************************************************/
        private void BuildBurialDetail ()
        {
            DataTable dt = (DataTable)dgv.DataSource;
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                if (String.IsNullOrWhiteSpace(location))
                    dt.Rows[i]["loc"] = "Unknown";
                funeralClass = dt.Rows[i]["funeral_classification"].ObjToString();
                if (String.IsNullOrWhiteSpace(funeralClass))
                    dt.Rows[i]["funeral_classification"] = "Other";
            }
            ...
            dx.Columns.Add("number", Type.GetType("System.Int32"));
            dx.Columns.Add("average", Type.GetType("System.Double"));

            DataRow dRow = null;

            int count = 0;
            int burialCount = 0;
            int cremationCount = 0;
            int otherCount = 0;
            int blankCount = 0;
            double burial = 0D;
            double cremation = 0D;
            ...
            double totalBurial = 0D;
            double totalCremation = 0D;
            double totalOther = 0D;
            double totalBlank = 0D;
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                if (location != lastLocation)
                {
                    ...
                    total = totalBurial + totalCremation + totalOther;

                    grandTotal += total;
                    grandCount += count;

                    dx = addNewRow(dx, lastLocation, total, count);

                    if ( totalBurial > 0D )
                        dx = addNewRow(dx, "  Burial", totalBurial, burialCount);

                    if ( totalCremation > 0D )
                        dx = addNewRow(dx, "  Cremation", totalCremation, cremationCount);

                    if ( totalOther > 0D )
                        dx = addNewRow(dx, "  Other", totalOther, otherCount);

                    if ( totalBlank > 0D )
                        dx = addNewRow(dx, "  BLANK", totalBlank, blankCount);
            ...
            dx = addNewRow(dx, "Grand Total", grandTotal, grandCount);

            G1.NumberDataTable(dx);
            dgv2.DataSource = dx;
        }
        /****************************************************************************************/
        private void BuildBurialArrangers()
        {
            DataTable dt = (DataTable)dgv.DataSource;
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                location = dt.Rows[i]["Funeral Arranger"].ObjToString();
                if (String.IsNullOrWhiteSpace(location))
                    dt.Rows[i]["Funeral Arranger"] = "Unknown";
                funeralClass = dt.Rows[i]["funeral_classification"].ObjToString();
                if (String.IsNullOrWhiteSpace(funeralClass))
                    dt.Rows[i]["funeral_classification"] = "Other";
        ...
        /****************************************************************************************/
        private DataTable addNewRow ( DataTable dx, string location, double value, int count )
        {
            ...
            dRow["total"] = value;
            dRow["number"] = count;
            dRow["average"] = average;
            dx.Rows.Add(dRow);

            return dx;
        }
        /****************************************************************************************/
        private DataTable SetupPageBreaks ( DataTable dt, string what )
        {
            DataView tempview = dt.DefaultView;
            tempview.Sort = what;
            dt = tempview.ToTable();
        ...
        private void cmbGroupBy_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            string group = box.Text.Trim();
            if (group == "None")
            {
                dt = SetupPageBreaks(dt, "Location Name");

                chkPageBreaks.Text = "Page Break at Locations";
                chkPageBreaks.Refresh();
            ...
            else if (group == "Arranger")
            {
                dt = SetupPageBreaks(dt, "Funeral Arranger");

                chkPageBreaks.Text = "Page Break at Arranger";
                chkPageBreaks.Refresh();
                gridMain.Columns["Location Name"].GroupIndex = -1;
            ...
            {
                dt = SetupPageBreaks(dt, "Funeral Director");

                chkPageBreaks.Text = "Page Break at Directors";
                chkPageBreaks.Refresh();
                gridMain.Columns["Location Name"].GroupIndex = -1;
                gridMain.Columns["Funeral Arranger"].GroupIndex = -1;
