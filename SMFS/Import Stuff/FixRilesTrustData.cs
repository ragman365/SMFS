...
{
    /***********************************************************************************************/
    public partial class FixRilesTrustData : DevExpress.XtraEditors.XtraForm
    {
        private string actualFile = "";
        /***********************************************************************************************/
        public FixRilesTrustData()
        {
            InitializeComponent();
        }
        /***********************************************************************************************/
        private void FixRilesTrustData_Load(object sender, EventArgs e)
        {
            btnFixTrusts.Hide();
            picLoader.Hide();
        }
        ...
        private void btnSelectFile_Click(object sender, EventArgs e)
        {
            ...
            string cmd = "Select * from `contracts` c LEFT JOIN `trust2013r` r ON c.`contractNumber` = r.`contractNumber` LEFT JOIN `payments` p ON c.`contractNumber` = p.`contractNumber` where c.`contractNumber` like 'RF%' AND p.`downPayment` > '0';";
            ...
            dt = dt.AsEnumerable().GroupBy(r => new { Col1 = r["contractNumber"] }).Select(g => g.OrderBy(r => r["contractNumber"]).First()).CopyToDataTable();
            for ( int i=0; i<dt.Rows.Count; i++)
            {
                tbb = dt.Rows[i]["beginningBalance1"].ObjToDouble();
                newTBB = tbb / 0.85D;
                trustPercent = dt.Rows[i]["trustPercent"].ObjToDouble();
                if (trustPercent != 100D)
                    newTBB = newTBB * trustPercent / 100D;
                newTBB = G1.RoundValue(newTBB);
                dt.Rows[i]["newTBB"] = newTBB;
            }
        ...
        private void gridMain_DoubleClick(object sender, EventArgs e)
        {
            ...
            double tbb = dr["beginningBalance"].ObjToDouble();
            this.Cursor = Cursors.WaitCursor;
        ...
        /***********************************************************************************************/
        private void btnFixTrusts_Click(object sender, EventArgs e)
        {
        }
        /***********************************************************************************************/
    }
